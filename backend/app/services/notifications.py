...
async def process_notifications(frequency: str):
    async with async_session() as session:
        stmt = (
            ...
            .join(User, UserPreference.user_id == User.id)
            .where(UserPreference.notification_frequency == frequency)
        )
        ...
        for pref, user in user_prefs:
            ...
            if pref.preferred_working_mode:
                job_stmt = job_stmt.where(
                    JobOffer.working_mode.ilike(f"%{pref.preferred_working_mode}%")
                )
            if pref.preferred_location:
                job_stmt = job_stmt.where(
                    JobOffer.location.ilike(f"%{pref.preferred_location}%")
                )
            ...
            if jobs:
                try:
                    ...
                    print(
                        f"Failed to send {frequency} notification to {user.email}: {e}"
                    )
